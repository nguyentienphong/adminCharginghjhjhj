...
class Controller_transaction_new extends Admin_Controller 
{
	public function __construct()
	{
	   ...
	   $this->load->helper('url');
		...
		$this->load->model('model_transactions_new');
	}
	...
	*/
	public function index()
	{
		...
		if(isset($_POST['submit_checkpending']))
		{
			...
			$this->render_template('transactions_new/index', null);
			return;
		}
		# start of page:
        ...
        $total_records = $this->model_transactions_new->countTransactionData($partner_username,$requestId,$serial,$pin, $fromDate, $toDate,$finalStatus);
		$config['base_url'] = base_url() . 'paging/config';
		
        if ($total_records > 0) 
        {
            // get current page records
            $params["results"] = $this->model_transactions_new->selectTransactionPaging($limit_per_page, $start_index,$partner_username,$requestId,$serial,$pin,$fromDate, $toDate,$finalStatus);
             
            $config['base_url'] = base_url() . 'Controller_transaction_new/index';
        ...
        $this->render_template('transactions_new/index', $params);
	}
	...
	public function reset_check_pending_time()
	{
		$fromDate = isset($_POST['fromDate']) ? $_POST['fromDate'] : '';
		$toDate = isset($_POST['toDate']) ? $_POST['toDate'] : '';
		$this->model_transactions_new->reset_check_pending($fromDate, $toDate);
